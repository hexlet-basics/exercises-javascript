...
Внешние кавычки можно и не менять: внутреннюю помечают обратным слэшем `\`, и тогда она не обрывает строку. Это называется экранированием.

<!-- lint-code-blocks: skip — экранирование это предмет примера, форматтер переписал бы его на кавычки без слэша -->

```text
console.log('it\'s JavaScript'); // апостроф экранирован, строка продолжается
```

Форматтер такую запись переписывает на кавычки без слэша, поэтому в готовом коде она встречается редко. Пригодится она тогда, когда в строке есть оба вида кавычек сразу.
