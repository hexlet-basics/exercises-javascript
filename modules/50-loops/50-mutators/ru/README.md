...
Линтеры (программы, проверяющие код) умеют ловить такой код. В oxlint за это отвечает правило [`no-plusplus`](https://oxc.rs/docs/guide/usage/linter/rules/eslint/no-plusplus.html): оно запрещает `++` и `--` и предлагает вместо них `+= 1` и `-= 1`. По умолчанию правило выключено, его включают в настройках проекта. Там, где его включили, код с инкрементом не пройдёт проверку линтера.
