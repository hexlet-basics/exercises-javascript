...
Здесь сначала вызывается `trim()`, который удаляет пробелы. Затем берётся срез `slice(7)`, начиная с восьмого символа. И уже после этого вызывается `toLowerCase()`. Такая запись читается слева направо и показывает весь путь преобразования данных в одной строке.
...
Число `6`, которое вернуло свойство `.length`, не умеет `.toUpperCase()` — это строковый метод. Зато у числа есть свои методы, например `.toString()`, поэтому `text.length.toString()` работает. Важно следить за тем, какой тип возвращает каждое звено цепочки.
